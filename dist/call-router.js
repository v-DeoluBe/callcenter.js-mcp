...
export class CallRouter {
        ...
        try {
            const response = await this.openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are a call routing assistant for an inbound phone line. " +
            ...
            const matchedTopic = parsed.topic_id && parsed.topic_id !== "none"
            ...
            getLogger().ai.debug(`Call router decision: topic=${decision.topic?.id || "none"} confidence=${decision.confidence} (${decision.reasoning})`);
            return decision;
        }
        catch (error) {
            getLogger().ai.error("Call router classification failed:", error instanceof Error ? error.message : String(error));
    ...
    async answer(topic, question) {
        if (topic.mcpEndpoint) {
            try {
                ...
                getLogger().ai.warn(`MCP endpoint for topic "${topic.id}" failed, falling back to static knowledge:`, error instanceof Error ? error.message : String(error));
            }
        }
        if (topic.knowledge) {
    ...
    async answerFromMcpEndpoint(endpoint, question) {
    ...
    async answerFromKnowledge(topic, question) {
        const response = await this.openai.chat.completions.create({
            model: "gpt-4o-mini",
            messages: [
                {
                    role: "system",
                    content: `You are answering a phone caller's question about "${topic.name}". ` +
